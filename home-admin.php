...
session_start();
    ...
    $sqlroom = "SELECT * FROM roomtbl WHERE status = '$tempOccupied'";
    $queryroom = mysqli_query($conn, $sqlroom);
    $occupied_rooms =  mysqli_num_rows($queryroom);

    $sqlroom = "SELECT * FROM roomtbl WHERE status = '$tempNotOccupied'";
    $queryroom = mysqli_query($conn, $sqlroom);
    $available_rooms =  mysqli_num_rows($queryroom);

    $sqlroom = "SELECT * FROM roomtbl";
    $queryroom = mysqli_query($conn, $sqlroom);
    ...
    $sqlroom = "SELECT * FROM roomtbl WHERE madeby = '$id' AND prof_seen = 0";
    $queryroom = mysqli_query($conn, $sqlroom);
...
<html lang="en">
    ...
    <body class="sb-nav-fixed">
        ...
        <nav class="sb-topnav navbar navbar-expand navbar-dark bg-dark">
            <!-- Navbar Brand-->
            <a class="navbar-brand ps-3" href="home-admin.php" >Admin Home</a>
            <!-- Sidebar Toggle-->
                                ...
                                <table class="datatable-table">
                                    <thead >
                                        <tr class="text-center">
                                            ...
                                            <th class="text-center">Attending Count</th>
                                            <th class="text-center">Not Attending Count</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                    <?php
                            ...
                            <div class="card-body text-center">
                                <table class="datatable-table">
                                    ...
                                    <?php
                                        ...
                                        $sqlroom = "SELECT * FROM usertbl";
                                        $queryroom = mysqli_query($conn, $sqlroom);
                                        $scheduled =  mysqli_num_rows($queryroom);
                                        $rooms_number = mysqli_num_rows($queryroom);
                                    
                                        while($resultroom = mysqli_fetch_assoc($queryroom)){
                                        $room_rid = $resultroom['id'];
                                        $room_fname = $resultroom['first_name'];
                                        $room_lname = $resultroom['last_name'];
                                        $room_stime= $resultroom['mobile_number'];
                                        $room_endtime = $resultroom['email'];
                                        $room_day = $resultroom['user_type'];
                                        $room_status = $resultroom['password'];
                                        $room_section = $resultroom['section'];
                                        $room_comb = $resultroom['year'];
                                        $room_attend = $resultroom['subjects'];
                                        echo '<tr>';
                                        echo '<td>' . $room_rid . '</td>';
                                        echo '<td>' . $room_fname . ' ' . $room_lname . '</td>';
                                        echo '<td>' . $room_comb . '</td>';
                                ...
                                <table class="datatable-table">
                                    <thead >
                                        <tr class="text-center">
                                            ...
                                            <th class="text-center">Request Title</th>
                                    ...
                                    <?php
                                        ...
                                        $sqlroom = "SELECT * FROM admin_request";
                                        $queryroom = mysqli_query($conn, $sqlroom);
                                        $scheduled =  mysqli_num_rows($queryroom);
                                        $rooms_number = mysqli_num_rows($queryroom);
                                        ...
                                        while($resultroom = mysqli_fetch_assoc($queryroom)){
                                        $room_rid = $resultroom['id'];
                                        $room_fname = $resultroom['sent_by'];
                                        $room_lname = $resultroom['req_type'];
                                        $room_stime= $resultroom['req_title'];
                                        $room_endtime = $resultroom['req_desc'];
                                        echo '<tr>';
                                        echo '<td>' . $room_rid . '</td>';
                                        echo '<td>' . $room_fname . '</td>';
                                        echo '<td>' . $room_comb . '</td>';
                                        echo '<td>' . $room_stime . '</td>';
                                        echo '<td>' . $room_endtime . '</td>';
                                        echo '</tr>';
